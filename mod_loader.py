import os, re, sys, json, shutil, time, tempfile, difflib, webbrowser, subprocess, logging, logging.handlers, zipfile, datetime, argparse, stat, atexit, errno, random, socket, hashlib
...
def _excepthook(exc_type, exc, tb):
    ...
    try:
        ...
        messagebox.showerror("Fatal Error", "An unexpected error occurred. See logs/modloader.log")
    except Exception:
        pass
...
def acquire_app_lock(lock_path: str = LOCK_PATH, retry_once: bool = True) -> bool:
    ...
    try:
        ...
        logger.exception("Atomic create of lock file failed, falling back to flock method")
    ...
    try:
        ...
        def _on_change(*_):
            ...
            try:
                ...
                new_lock_path = os.path.join(os.path.realpath(new_wr), ".modloader.lock")
                fd_new = None
                attempts = 5
                ...
                if not fd_new:
                    ...
                    app.append_log(f"Failed to acquire workroot lock for {new_wr}. Another instance may hold it.")
                    try:
                        from tkinter import messagebox
                        messagebox.showwarning("Lock Failed",
                ...
                try:
                    try:
                        ...
                        logger.exception("release_platform_lock failed during workroot switch")
                    release_app_lock()
                except Exception:
                ...
                try:
                    ...
                    try:
                        if old_lock_path and os.path.exists(old_lock_path):
                            try:
                                ...
                                app.append_log(f"Removed old lock file: {old_lock_path}")
                            except Exception:
                                # if removal fails ignore; it will be cleaned later
                                logger.exception("Failed removing old lock file: %s", old_lock_path)
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        logger.exception("Failed to re-acquire global lock after adopting new lock failed")
                    app.append_log(f"Failed to adopt workroot lock for {new_wr}; retained previous lock")
...
def headless_dryrun(original_root, work_root, instructions):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        bundle = save_dryrun_artifact(sim_log, temp_work_root, original_root, out_dir=LOG_DIR)
        return bundle
...
def _cli_main(argv=None):
    ...
    p.add_argument("--workroot", "-w", required=True, help="Work root directory (unused, informational)")
    p.add_argument("--instructions", "-i", required=False, help="JSON file with instructions list")
    p.add_argument("--out", "-O", required=False, help="Optional output path for created support bundle (.zip)")
    ...
    try:
        ...
        if args.out and bundle:
            ...
            try:
                ...
                print(f"Error: failed copying bundle to {args.out}: {e}", file=sys.stderr)
                return 3
        else:
            print(bundle or "")
        return 0
...
def get_function_block(lines: List[str], func_name: str) -> Optional[Tuple[int, int]]:
    ...
    while end >= body_start and (_is_line_comment(lines[end]) or not lines[end].strip()):
        end -= 1
...
def parse_mod_config(mod_path: str) -> Optional[Dict[str, Any]]:
    ...
    config_file = next((f for f in ['mod.mos', 'mod.ini', 'mod.txt'] if os.path.exists(os.path.join(mod_path, f))), None)
    if not config_file:
        return None
...
def generate_patch_plan(mod_path: str, mod_config: Dict[str, Any]) -> List[Tuple[str, str, Tuple]]:
    """
    ...
    for line in sections.get('VariablePatch', []):
        ...
        if mode not in ('add', 'replace', 'create'):
            raise ValueError(f"VariablePatch in mod '{mod_name}' must include '; mode=add|replace|create' in line: {line}")
        # Disallow rename attempts on replace
        if mode == 'replace' and s_var and s_var != t_var:
            raise ValueError(f"VariablePatch replace in mod '{mod_name}' attempts rename '{t_var}' -> '{s_var}'. Keep original name.")
    ...
    for line in sections.get('DataPatch', []) + sections.get('DataAdd', []):
        ...
        s_path = os.path.join(mod_path, s_res)
        plan.append((mod_name, 'VariablePatch', (t_res, t_var, s_path, s_var, 'create')))
...
def validate_mod_config(mod_config: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        for line in sections.get('VariablePatch', []):
            ...
            if 'mode' not in meta:
                return False, f"VariablePatch missing '; mode=add|replace|create' in line: {line}"
            s_path = os.path.join(mod_path, s_res)
            ...
            if meta.get('mode') == 'replace' and s_var and s_var != t_var:
                return False, f"VariablePatch replace attempts rename '{t_var}' -> '{s_var}' in line: {line}"
...
def read_source_for_patching(work_path):
    ...
    if not p.exists():
        ...
        raise FileNotFoundError(f"Target file does not exist in working directory: {work_path}")
...
def atomic_copy_with_single_bak(src: str, dst: str):
    ...
    if dst_p.exists() and not bak.exists():
        ...
        try:
            ...
            if os.path.exists(tmp_bak): os.remove(tmp_bak)
...
def lazy_copy_file(original_root: str, work_root: str, res_path: str) -> Tuple[str, str]:
...
def patch_variable(original_root: str, work_root: str,
                   target_res: str, target_var: str,
                   source_path: str, source_var: str,
    ...
    try:
        ...
        if not src_range:
            log.append(f"ERROR: Source var '{source_var}' not found in '{source_path}'.")
            return log
        ...
        if mode == 'replace':
            if not tgt_range:
                log.append(f"ERROR: Target var '{target_var}' not found for replace in {target_res}. Skipping.")
                return log
            ...
            if src_block and source_var != target_var:
                ...
                pat = re.compile(rf"(^\s*(var|const)\s+){re.escape(source_var)}(\s*[:=])")
                src_block[0] = pat.sub(rf"\1{target_var}\3", src_block[0], count=1)

            new_lines = target_lines[:tgt_range[0]] + src_block + target_lines[tgt_range[1] + 1:]
            ensure_within(work_root, work_path)
        ...
        if mode == 'add':
            if not tgt_range:
                log.append(f"ERROR: Target var '{target_var}' not present; 'add' requires existing var. Skipping.")
                return log
            ...
            ensure_within(work_root, work_path)
            atomic_write_with_backup(work_path, ''.join(new_lines))
            log.append(f"SUCCESS: Appended {len(inner)} lines into '{target_var}' in {target_res}.")
            return log

        if mode == 'create':
            if tgt_range:
                log.append(f"ERROR: Target var '{target_var}' already exists; DataAdd/create will not overwrite. Skipping.")
                return log
...
def patch_function(original_root: str, work_root: str,
                   target_res: str, target_func: Optional[str],
                   source_path: str, source_func: str,
    ...
    try:
        ...
        if effective_mode == 'create':
            ...
            if get_function_block(target_lines, target_func):
                log.append(f"ERROR: Target function '{target_func}' already exists. 'create' mode will not overwrite. Skipping.")
                return log
            ...
            if source_sig_idx == -1:
                log.append(f"ERROR: Source function '{source_func}' not found in '{source_path}'. Skipping create.")
                return log
            ...
            if source_func != target_func:
                ...
                new_func_block[0] = pat.sub(rf"\1{target_func}\2", new_func_block[0], count=1)
                
            # Append the new function to the end of the file
            new_lines = target_lines + ['\n'] + new_func_block
            atomic_write_with_backup(work_path, ''.join(new_lines))
            log.append(f"SUCCESS: Created new function '{target_func}' in '{target_res}'.")
            return log
        ...
        if target_sig_line_index == -1:
            log.append(f"ERROR: Target function '{target_func}' not found in '{target_res}'. Skipping.")
            return log
        ...
        if effective_mode == 'replace':
            ...
            if source_sig_idx == -1:
                log.append(f"ERROR: Source function '{source_func}' not found for replace. Skipping.")
                return log
            
            start_idx = target_sig_line_index
            end_idx = target_body_range[1] if target_body_range else start_idx
            
            source_end_idx = source_body_range[1] if source_body_range else source_sig_idx
            patch_content = source_lines[source_sig_idx : source_end_idx + 1]

            # Rename if necessary
            if source_func != target_func:
                pat = re.compile(rf"(^\s*func\s+){re.escape(source_func)}(\s*\(.*?\):)")
                patch_content[0] = pat.sub(rf"\1{target_func}\2", patch_content[0], count=1)
            
            new_lines = target_lines[:start_idx] + patch_content + target_lines[end_idx + 1:]

        else: # prefix or postfix
            if not target_body_range:
                log.append(f"ERROR: Original function body for '{target_func}' is empty. Cannot wrap.")
                return log
            if not source_body_range:
                log.append(f"ERROR: Patch function body for '{source_func}' is empty. Cannot wrap.")
                return log

            original_body_lines = target_lines[target_body_range[0] : target_body_range[1] + 1]
            patch_body_lines = source_lines[source_body_range[0] : source_body_range[1] + 1]
            body_indent = _leading_whitespace(target_lines[target_sig_line_index]) + '    '
            wrapper_body = []
            wrapper_body.append(f"{body_indent}#--- START {effective_mode.upper()} PATCH: {Path(source_path).name}::{source_func} ---\n")
            ...
            wrapper_body.append(f"{body_indent}#--- END {effective_mode.upper()} PATCH ---\n")
            
            start_idx = target_sig_line_index + 1
            end_idx = target_body_range[1]
            new_lines = target_lines[:start_idx] + wrapper_body + target_lines[end_idx + 1:]

        # --- FIX: Ensure file is always written for replace/prefix/postfix ---
        if new_lines:
            ensure_within(work_root, work_path)
            atomic_write_with_backup(work_path, ''.join(new_lines))
            log.append(f"SUCCESS: Function '{target_func}' patched with {effective_mode.upper()} in '{target_res}'.")
...
def patch_script_replace(original_root: str, work_root: str, target_res: str, source_path: str) -> List[str]:
    ...
    try:
        ...
        log.append(f"SUCCESS: FileReplace: '{target_res}' replaced by '{Path(source_path).name}'.")
        return log
...
def run_patcher(original_root: str, work_root: str, patch_plan: List[Tuple[str, str, Tuple]]) -> List[str]:
    """
    ...
    for mod_name, op, details in other_ops:
        ...
        try:
            if op == 'FileReplace':
                ...
                log.extend(patch_script_replace(original_root, work_root, target_res, source_path))
                applied_files.add(_res_to_path(target_res))
                applied_ops.append({"mod": mod_name, "op": op, "target": target_res, "source": source_path})
            elif op == 'FunctionPatch':
                try:
                ...
                try:
                    log.extend(patch_function(original_root, work_root, t_res, t_func, s_path, s_func, mode=mode))
                    applied_files.add(_res_to_path(t_res))
                    applied_ops.append({"mod": mod_name, "op": op, "target": f"{t_res}::{t_func}", "source": f"{s_path}::{s_func}", "mode": mode})
                except Exception as e:
                    log.append(f"FATAL ERROR while processing FunctionPatch for {mod_name}: {e}")
                    applied_ops.append({"mod": mod_name, "op": op, "status": "error", "notes": str(e)})
            else:
                 log.append(f"WARNING: Unknown non-variable operation '{op}' from {mod_name}. Skipped.")
    ...
    by_target: Dict[Tuple[str, str], List[Tuple[str, str, str, str]]] = defaultdict(list)
    for mod_name, op, details in var_ops:
        try:
            t_res, t_var, s_path, s_var, mode = details
        except ValueError:
            log.append(f"ERROR: Malformed VariablePatch details from {mod_name}: {details}")
            continue
    ...
    for (t_res, t_var), ops in by_target.items():
        ...
        for mod_name, s_path, s_var, mode in ops:
            if mode == 'replace':
                ...
                lines = patch_variable(original_root, work_root, t_res, t_var, s_path, s_var, mode='replace')
                log.extend(lines)
                applied_ops.append({"mod": mod_name, "op": "VariablePatch", "mode": "replace", "target": f"{t_res}::{t_var}", "source": f"{s_path}::{s_var}"})
                applied_files.add(_res_to_path(t_res))
        # CREATE
        for mod_name, s_path, s_var, mode in ops:
            if mode in ('create', 'dataadd'):
                log.append(f"--- Applying Variable CREATE from {mod_name} ---")
                lines = patch_variable(original_root, work_root, t_res, t_var, s_path, s_var, mode='create')
                log.extend(lines)
                applied_ops.append({"mod": mod_name, "op": "VariablePatch", "mode": "create", "target": f"{t_res}::{t_var}", "source": f"{s_path}::{s_var}"})
                applied_files.add(_res_to_path(t_res))
        # ADD
        for mod_name, s_path, s_var, mode in ops:
            if mode == 'add':
                log.append(f"--- Applying Variable ADD from {mod_name} ---")
                lines = patch_variable(original_root, work_root, t_res, t_var, s_path, s_var, mode='add')
                log.extend(lines)
                applied_ops.append({"mod": mod_name, "op": "VariablePatch", "mode": "add", "target": f"{t_res}::{t_var}", "source": f"{s_path}::{s_var}"})
                applied_files.add(_res_to_path(t_res))
    ...
    try:
        ...
        log_content += '\n'.join(log) + '\n'
        log_content += '--- end run ---\n'
        atomic_replace(log_path, log_content) # Use replace instead of append for clean log
    except Exception:
        pass
...
def analyze_mods_for_conflicts(mod_configs: List[Dict[str, Any]]) -> Dict[str, List[Tuple[str, str, Any]]]:
    """
    ...
    for key, instrs in targets.items():
        ...
        if key.startswith('Variable::'):
            replace_count = sum(1 for _, _, d in instrs if isinstance(d, tuple) and len(d) >= 5 and d[4] == 'replace')
            create_count  = sum(1 for _, _, d in instrs if isinstance(d, tuple) and len(d) >= 5 and d[4] in ('create','dataadd'))
            add_count     = sum(1 for _, _, d in instrs if isinstance(d, tuple) and len(d) >= 5 and d[4] == 'add')
...
class ResolveDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        tk.Label(master, text="Conflicts detected. Later mods win. Review and reorder or disable mods.", 
                 font=('Inter', 10, 'bold')).pack(pady=6)
        ...
        for target_key, instructions in self.conflicts.items():
            ...
            lb = tk.Listbox(mods_frame, height=min(6, len(instructions)), exportselection=False)
            lb.pack(side='left', fill='x', expand=True)
            # attach a small frame with quick action buttons
            btnf = ttk.Frame(mods_frame)
            btnf.pack(side='right', fill='y', padx=4)
            ttk.Button(btnf, text="Open Folder", command=lambda l=lb: self._open_selected_mod_folder(l)).pack(fill='x', pady=2)
            ttk.Button(btnf, text="Toggle Enable", command=lambda l=lb: self._toggle_selected_mod(l)).pack(fill='x', pady=2)
            ttk.Button(btnf, text="Select in Main List", command=lambda l=lb: self._select_in_main_list(l)).pack(fill='x', pady=2)
        ...
        ttk.Label(master, text="Reorder mods (last wins):", font=('Inter', 10, 'bold')).pack(pady=(8,4))
        ...
        ttk.Button(reorder_buttons, text="Move Down", command=self.move_down).pack(pady=6)
        ttk.Button(reorder_buttons, text="Reset Order", command=self.reset_order).pack(pady=6)
    ...
    def move_down(self):
        ...
        if not sel: return
        i = sel[0]
        if i >= self.list_box.size()-1: return
        val = self.list_box.get(i)
        self.list_box.delete(i)
        self.list_box.insert(i+1, val)
        self.list_box.selection_set(i+1)
    ...
    def buttonbox(self):
        box = ttk.Frame(self)
        ttk.Button(box, text="Resolve & Re-Patch", width=15, command=self.ok).pack(side=tk.LEFT, padx=5, pady=5)
...
class _ToolTip:
    ...
    def _show(self):
        ...
        lbl = ttk.Label(self.tw, text=self.text, relief='solid', borderwidth=1, padding=(6,3))
        lbl.pack()
...
class App(tk.Tk):
    def __init__(self):
        ...
        style.configure("Accent.TButton", foreground="green", background="black", font=('Arial', 10, 'bold'))
        style.map("Accent.TButton", background=[('active', 'dark green')], foreground=[('active', 'white')])
        self.config = DEFAULTS.copy()
        self.mod_configs = [] # Stores parsed mod info
        self.instructions = [] # The final, ordered list of patches (mod_name, op, details)
    ...
    def setup_ui(self):
        ...
        for key, default in DEFAULTS.items():
            if key == 'mos_module': continue

            tk.Label(config_frame, text=key.replace('_', ' ').title() + ":").grid(row=row, column=0, sticky='w', padx=5, pady=2)
            ...
            if key.endswith('_dir') or key == 'launch_override':
                ...
                command = self.browse_directory if key.endswith('_dir') else self.browse_file
                ttk.Button(config_frame, text="Browse", command=lambda k=key, v=var, cmd=command: cmd(v)).grid(row=row, column=2, padx=5, pady=2)
            else:
        ...
        # 2. Mod List Section
        mod_list_frame = ttk.LabelFrame(top_frame, text="Loaded Mods (Order determines Patch Priority - Last Wins)", padding="10")
        ...
        ttk.Button(list_controls_frame, text="Refresh Mods", command=self.load_mods).pack(side='left', padx=5)
        self.conflict_label = tk.Label(list_controls_frame, text="No Conflicts Detected", fg="green")
        self.conflict_label.pack(side='left', padx=10)
        
        mod_list_controls = ttk.Frame(mod_list_frame)
        mod_list_controls.pack(fill='x', pady=5)
        ttk.Button(mod_list_controls, text="Move Up", command=lambda: self.move_selected_mod(-1)).pack(side='left', padx=5)
        ttk.Button(mod_list_controls, text="Move Down", command=lambda: self.move_selected_mod(1)).pack(side='left', padx=5)
        ttk.Button(mod_list_controls, text="Toggle Enable", command=self.toggle_selected_mod).pack(side='left', padx=5)
        btn_export = ttk.Button(mod_list_controls, text='📤', width=3, command=self.export_mod_order)
        btn_export.pack(side='left', padx=5)
        _TT = _ToolTip
        _TT(btn_export, "Export current mod order to a JSON file")
        btn_import = ttk.Button(mod_list_controls, text='📥', width=3, command=self.import_mod_order)
        btn_import.pack(side='left', padx=5)
        _TT(btn_import, "Import mod order from a JSON file")
        ttk.Button(mod_list_controls, text="Resolve Conflicts", command=self.open_resolve_dialog).pack(side='right', padx=5)
        ttk.Button(mod_list_controls, text="Rollback Working Dir", command=self.rollback_working_dir).pack(side='right', padx=5)
        ttk.Button(mod_list_controls, text="Open Working Dir", command=lambda: webbrowser.open(safe_norm(self.vars['work_root_dir'].get()))).pack(side='right', padx=5)
        ttk.Button(mod_list_controls, text="View Runtime Manifest", command=self.view_runtime_manifest).pack(side='right', padx=5)
        ...
        self._btn_up = ttk.Button(self._arrow_frame, text='▲', width=2, command=lambda: self.move_selected_mod(-1))
        self._btn_down = ttk.Button(self._arrow_frame, text='▼', width=2, command=lambda: self.move_selected_mod(1))
        ...
        ttk.Button(action_frame, text="Apply Patch to Working Dir", command=self.run_patcher_action).pack(side='left', fill='x', expand=True, padx=5)
        ttk.Button(action_frame, text="Simulate & Diff Patches", command=self.simulate_and_diff_action).pack(side='left', fill='x', expand=True, padx=5)
        ttk.Button(action_frame, text="Save Diff", command=self.save_diff_to_file).pack(side='left', padx=5)
        ttk.Button(action_frame, text="START GAME", command=self.start_game_action, style="Accent.TButton").pack(side='right', padx=10)
    ...
    def _on_mod_selection_change(self, _ev=None):
        ...
        try:
            ...
            if not bbox:
                ...
                return
            x, y, w, h = bbox
            # compute absolute position in parent frame
            abs_x = self.mod_list_box.winfo_rootx()
            abs_y = self.mod_list_box.winfo_rooty()
            parent_x = self.mod_list_box.master.winfo_rootx()
            parent_y = self.mod_list_box.master.winfo_rooty()
    ...
    def _on_mod_double_click(self, event=None):
        ...
        self.append_log(f"Mod '{mod['Name']}' {'enabled' if mod['Enabled'] else 'disabled'} via double-click.")
    ...
    def load_mods(self, mod_configs_override: Optional[List[Dict[str, Any]]] = None):
            ...
            for item in os.listdir(mods_dir):
                ...
                if os.path.isdir(mod_path):
                    ...
                    if config:
                        ...
                        if not valid:
                            config['Errors'] = err
                            self.append_log(f"ERROR: Mod '{config['Name']}' invalid: {err}")
                        else:
                            config['Errors'] = None
        ...
        if invalid_count:
            ...
            messagebox.showerror("Invalid Mods Detected", "Some mods failed validation and were skipped:\n\n" + "\n\n".join(msg_lines))

        self.update_conflict_status()
        self.append_log(f"Loaded {len(self.mod_configs)} mods ({invalid_count} invalid).")
        self._arrow_frame.place_forget()
    ...
    def rollback_working_dir(self):
        ...
        work_root = safe_norm(self.vars['work_root_dir'].get()) if 'work_root_dir' in self.vars else None
        ...
        if not bak_list:
            resp = messagebox.askyesno("Rollback", "No .bak files found. Remove entire working directory?")
            if resp:
                try:
        ...
        lbl = tk.Label(preview, text="Select .bak files to restore (checked) or choose Remove Working Directory.")
        ...
        def _restore_selected():
            ...
            if not selected:
                ...
                return
            confirm = messagebox.askyesno("Confirm Restore", f"Restore {len(selected)} files from .bak?")
            if not confirm:
                return
            restored = 0
            errors = []
            for rel in selected:
                ...
                try:
                    ...
                    if not os.path.commonpath([work_root, bak]).startswith(os.path.normpath(work_root)):
                        raise RuntimeError("path outside work_root")
                    atomic_write_copy(bak, orig)
                    restored += 1
                    self.append_log(f"Rollback: restored {rel} -> {os.path.relpath(orig, work_root)}")
                except Exception as e:
        ...
        def _remove_work_root():
            confirm = messagebox.askyesno("Confirm Remove", f"Remove entire working directory: {work_root}?")
            if not confirm:
                return
            try:
                shutil.rmtree(work_root)
        ...
        btn_restore = tk.Button(btn_frame, text="Restore Selected", command=_restore_selected)
        btn_restore.pack(side='left', padx=6)
        btn_remove = tk.Button(btn_frame, text="Remove Working Directory", command=_remove_work_root)
    ...
    def create_support_bundle(self):
        ...
        try:
            ...
            out = filedialog.asksaveasfilename(defaultextension=".zip", initialfile=os.path.basename(default))
            if not out:
                return
        except Exception:
        ...
        try:
            with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                ...
                if work_root:
                    ...
                    if os.path.exists(candidate):
                        zf.write(candidate, os.path.join("work_root", "runtime_manifest.json"))
                ...
                for candidate in [
                    ...
                    if candidate and os.path.exists(candidate):
                        zf.write(candidate, os.path.join("work_root", os.path.basename(candidate)))
            ...
            try:
                messagebox.showerror("Support Bundle Error", f"Failed to create bundle: {e}")
            except Exception:
                pass
    ...
    def export_mod_order(self):
        ...
        if not self.mod_configs:
            ...
            return
        save_path = filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("JSON","*.json")], title="Save mod order")
        if not save_path:
            return
        data = []
        ...
        try:
            ...
            messagebox.showinfo("Export Complete", f"Mod order exported to:\n{save_path}")
    ...
    def import_mod_order(self):
        ...
        path = filedialog.askopenfilename(filetypes=[("JSON","*.json")], title="Import mod order")
        if not path:
            return
        try:
            with open(path, 'r', encoding='utf-8') as f:
    ...
    def update_patch_instructions(self):
        ...
        for mod_config in self.mod_configs:
            ...
            try:
                ...
                skipped.append(mod_config['Name'])
                self.append_log(f"ERROR: Failed to generate patch plan for '{mod_config['Name']}': {e}")
        self.append_log(f"Generated {len(self.instructions)} patch instructions. Skipped mods: {', '.join(skipped) if skipped else 'none'}.")
    ...
    def update_conflict_status(self):
        ...
        if conflicts:
            count = len(conflicts)
            self.conflict_label.config(text=f"{count} Conflict{'s' if count > 1 else ''} Detected! (Click to Resolve)", fg="red")
    ...
    def open_resolve_dialog(self):
        ...
        conflicts = analyze_mods_for_conflicts(self.mod_configs)
        if not conflicts:
            messagebox.showinfo("No Conflicts", "No critical conflicts were found. You can reorder mods using Move Up/Down.")
            return
    ...
    def resolve_dialog_callback(self, new_mod_configs):
        ...
        messagebox.showinfo("Resolution Complete", "Mod order updated. Patch instructions regenerated.")
    ...
    def toggle_selected_mod(self):
        ...
        try:
            ...
            try:
                self.mod_list_box.itemconfig(idx, fg='gray' if not mod['Enabled'] else 'black')
            except Exception:
                pass
            self.mod_list_box.selection_set(idx)
            self.append_log(f"Mod '{mod.get('Name')}' {'enabled' if mod['Enabled'] else 'disabled'}.")
    ...
    def toggle_mod_enabled_by_name(self, mod_name: str):
        ...
        for idx, m in enumerate(self.mod_configs):
            if m['Name'] == mod_name:
                ...
                self.append_log(f"Mod '{mod_name}' {'enabled' if m['Enabled'] else 'disabled'} (toggle).")
    ...
    def run_patcher_action(self):
        ...
        if not os.path.isdir(original_root):
            messagebox.showerror("Error", f"Original game directory not found: {original_root}")
            return
            
        if not self.instructions:
            messagebox.showwarning("Warning", "No mod instructions loaded. Load mods first.")
            return
        ...
        self.append_log(f"Starting lazy patching process...")
        patch_log = run_patcher(original_root, work_root, self.instructions)
        
        for line in patch_log:
            self.append_log(f"PATCH: {line}")

        messagebox.showinfo("Patch Complete", "Patching is complete. The working directory now contains only modified files.")
    ...
    def start_game_action(self):
        ...
        if launch_override:
            ...
            if not os.path.exists(work_exe_path):
                if not os.path.exists(original_exe_path):
                    messagebox.showerror("Launch Error", f"Game executable not found in original directory: {original_exe_path}")
                    return
                self.append_log(f"Copying game executable to working directory...")
                try:
                    ...
                    messagebox.showerror("Launch Error", f"Failed to copy executable to working directory: {e}")
                    return
        
            game_exe_path = work_exe_path

        if not os.path.exists(game_exe_path):
            messagebox.showerror("Launch Error", f"Game executable not found at: {game_exe_path}")
            return
        ...
        self.append_log(f"--- Attempting to Launch Game ---")
    ...
    def simulate_and_diff_action(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                if os.path.exists(manifest_path):
                    with open(manifest_path, 'r', encoding='utf-8') as mf:
                        ...
                        self.append_log(f"Used runtime_manifest.json for diff (found {len(patched_rel_paths)} files).")
                else:
                    ...
                    if patched_rel_paths:
                        self.append_log(f"Fallback: parsed sim_log for {len(patched_rel_paths)} files.")
            except Exception as e:
                self.append_log(f"Warning: failed to read runtime_manifest.json or parse sim_log: {e}")
            ...
            if not patched_rel_paths:
                self.diff_txt.insert(tk.END, "No files were modified during the simulation.")
                return
            ...
            for rel in dedup_paths:
                ...
                diff_iter = difflib.unified_diff(orig_lines, patched_lines,
            ...
            try:
                ...
                try:
                    self._save_dryrun_artifact(sim_log, temp_work_root, original_root, combined_diff=combined)
                except TypeError:
    ...
    def _save_dryrun_artifact(self, sim_log, temp_work_root, original_root, combined_diff: str = None):
        """Persist sim_log and runtime_manifest.json to logs/dryrun_TIMESTAMP/ and zip it."""
        ts = datetime.datetime.now(datetime.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    ...
    def save_diff_to_file(self):
        ...
        try:
            ...
            path = filedialog.asksaveasfilename(defaultextension=".patch",
    ...
    def view_runtime_manifest(self):
        ...
        if not os.path.exists(manifest_path):
            messagebox.showinfo("Runtime Manifest", f"No runtime_manifest.json found in {work_root}")
            return
        try:
...
def main_entry():
    ...
    if not got_lock:
        ...
        if len(sys.argv) > 1:
            print("Another Mod Loader instance is already running. Exiting.", file=sys.stderr)
            return 2
        else:
            try:
                from tkinter import messagebox
                messagebox.showerror("Already Running", "Another Mod Loader instance is already running. Close it and try again.")
            except Exception:
                print("Another Mod Loader instance is already running. Exiting.", file=sys.stderr)
            return 2
    ...
    try:
        ...
        try:
            messagebox.showerror("Startup Error", f"Failed to start GUI. See log: {os.path.join(LOG_DIR,'modloader.log')}\n\n{e}")
        except Exception:
            pass
        return 1
